...
regDate_and_status <- read_excel("Desktop/PMC Github/RiderReg23/Jarrett_StatsbyRouteStart.xlsx", 
              sheet = "Reg Date and Status")



# Merge Data --------------------------------------------------------------

merged_data <- routes_2022 %>%
...
merged_data<- merged_data %>% 
  ...
  mutate(
    ...
    `Route Start` = case_when(
      ...
      str_detect(Description, "2-Day") & str_detect(Description, "Wellesley") ~ "Wellesley Start",
  ...
  mutate(TeamID = ifelse(is.na(TeamID), "null", TeamID)) %>% 
  mutate(TeamID = ifelse(TeamID == "null", 0, 1))
...
aggregate_start_routes <- merged_data %>% 
  ...
  summarize(`Number of Riders` = n(),
            ...
            `Average Reg Date` = mean(RegDate),
            `Percent Male` = (sum(Gender == "M")/n())*100,
            `Percent Team` = (sum(!is.null(TeamID))/n())*100,
...
write_xlsx(merged_data, "merged_data11_1.xlsx")
